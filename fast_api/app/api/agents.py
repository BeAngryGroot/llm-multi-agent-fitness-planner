...
class MealPlannerAgent:
    ...
    async def generate_meal_plan(
        ...
        try:
            # For Tongyi, we'll use regular output and parse it
            response = self.llm.invoke(messages)
            # Note: Tongyi doesn't support structured output like OpenAI, so we'll need to adjust
            # For now, we'll return a simplified meal plan
            meal_plan = {
                "user_id": profile.user_id,
                "plan_name": f"Personalized Meal Plan for {profile.fitness_goal}",
                "days": request.days,
                "target_macros": {
...
class WorkoutPlannerAgent:
    ...
    async def generate_workout_plan(
        ...
        try:
            # For Tongyi, we'll use regular output
            response = self.llm.invoke(messages)
            workout_plan = {
                "user_id": profile.user_id,
                "plan_name": f"Personalized Workout Plan for {profile.fitness_goal}",
                "split_type": request.split_type,
                "training_style": request.training_style,
                "days_per_week": request.days_per_week,
                "duration_minutes": request.duration_minutes,
                "generated_at": datetime.now(),
                "workout_plan_text": response,
            }
